...
const SITE_BASE_HREF = "https://elivestit.github.io/cached-airports/";

const existingStateReplacements = {
  files: "dist/cached-airports/ngsw-worker.js",
...
const baseHrefInstances = {
  files: "dist/cached-airports/ngsw.json",
...
try {
  ...
  console.log(
    "Replacements of EXISTING_CLIENTS_ONLY states: ",
    existingInstances.join(", ")
  );
} catch (error) {
  console.error(
    "Error occurred while replacing EXISTING_CLIENTS_ONLY states: ",
    error
  );
}
...
try {
  ...
  if (foundBaseHref && foundBaseHref.length > 0) {
    try {
      ...
      console.error(
        "Error occurred while overriding default service worker URL behavior: ",
        error
      );
    }
  } else {
    console.log(
      "baseHref was not set; no URL matching changes needed",
      foundBaseHref
    );
  }
} catch (error) {
